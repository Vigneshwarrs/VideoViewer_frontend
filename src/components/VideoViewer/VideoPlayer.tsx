  ...
  useEffect(() => {
      ...
      const handleSourceOpen = () => {
        ...
        try {
          ...
          sourceBuffer.addEventListener('abort', (e) => {
            console.log('⚠️ SourceBuffer operation aborted:', e);
            isProcessing = false;
          });
      ...
      if (chunk) {
        try {
          ...
          if (uint8View[0] === 0x00 && uint8View[1] === 0x00 && uint8View[2] === 0x00 && uint8View[3] === 0x01) {
            ...
            console.log('⚠️ Unknown chunk format - may cause decode errors');
          }
          ...
          if (error.name === 'QuotaExceededError') {
            ...
            console.log('⚠️ Invalid state, retrying...');
            setTimeout(() => {
              isProcessing = false;
              processChunkQueue();
            }, 100);
    ...
    const handleQuotaExceeded = () => {
      ...
      try {
        ...
        if (removeEnd > bufferStart) {
          console.log(`🗑️ Removing buffer: ${bufferStart.toFixed(2)}s-${removeEnd.toFixed(2)}s`);
    ...
    const setupVideoEvents = () => {
      ...
      videoElement.addEventListener('stalled', () => {
        console.log('⚠️ Video download stalled');
      });
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <div className="absolute right-0 top-1/2 transform translate-x-1/2 -translate-y-1/2 w-3 h-3 bg-white rounded-full shadow-lg opacity-0 group-hover/progress:opacity-100 transition-opacity" />
          </div>
        </div>
